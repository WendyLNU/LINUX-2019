...
if(!($EntName = $_POST['EntName'])){$EntName = 'Entname';};
if(!($EntNum = $_POST['EntNum'])){$EntNum = '0000';};
if(!($area = $_POST['area'])){$area = '86';};
if(!($phone = $_POST['phone'])){$phone = '未填写';};
if(!($email = $_POST['email'])){$email = '未填写';};
if(!($pw1 = $_POST['pw1'])){$pw1 = 'password';};

if(!($address = $_POST['address'])){$address = '未填写';};
if(!($potalcode = $_POST['potalcode'])){$potalcode = '000000';};
if(!($pay = $_POST['pay'])){$pay = '未填写';};
if(!($payid = $_POST['payid'])){$payid = '未填写';};
if(!($paypwd = $_POST['paypwd'])){$paypwd = '未填写';};

setcookie('EntName',$EntName);
setcookie('EntNum',$EntNum);
...
echo "
  <h5>企业名称：</h5><p>$EntName</p>
  <h5>企业编号：</h5><p>$EntNum</p>
  <h5>电&nbsp&nbsp话：</h5><p>$area $phone</p>
...
?>
  ...
  <div id="bottons">
    ...
    <a href="idx_login_ins2.php"><button>提交</button></a>
  </div>
</div>
