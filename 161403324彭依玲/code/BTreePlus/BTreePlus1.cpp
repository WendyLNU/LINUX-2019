...
static KeyType Unavailable = INT_MIN;  //INT-MIN的值是什么？
						//在limits.h中定义的值，最小整数-2147483648
 
/* 生成一个节点并初始化 */
static BPlusTree MallocNewNode(){
    BPlusTree NewNode;
    int i;
    NewNode = (struct BPlusNode *)malloc(sizeof(struct BPlusNode)); //申请BPlusNode类型的内存空间
    if (NewNode == NULL)
        exit(EXIT_FAILURE);//0表示程序寿终正寝，1表示程序死于非命
    i = 0;
    while (i < M + 1){
...
static Position FindMostRight(Position P){
    ...
    while (Tmp != NULL && Tmp->Children[Tmp->KeyNum - 1] != NULL) {
		...
		//因为关键字个数是由0开始数的，所以最后一个应为KeyNum-1
    }
    return Tmp;
}
...
static BPlusTree SplitNode(Position Parent,Position X,int i){
    ...
    while (j < Limit){
        if (X->Children[0] != NULL){
			//X的孩子不为空的话，分一半的孩子给新节点
            NewNode->Children[k] = X->Children[j];
            X->Children[j] = NULL;
        }
        NewNode->Key[k] = X->Key[j];	//分一半的Key值给新节点
        X->Key[j] = Unavailable;
